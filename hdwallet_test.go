...
const (
	seedKey                   = "test_seed_key"
	pwSeed                    = "test_pw"
	pwBitcoinMasterKey        = "Bitcoin seed"
	mnemonic12                = "feature burst special hospital miracle gesture alien pact large pumpkin media thought"
	mnemonic24                = "couch amount cycle wall egg tank cabbage tower poem fringe yard birth when balcony vapor blade curtain seven lab light enhance weapon hero team"
	idx1               uint32 = 1
	idx2               uint32 = 5
	idx3               uint32 = 10
)
...
func TestMnemonicNew(t *testing.T) {
	fn_test := func(mnemonicLen int) {
		entropy, err := EntropyNew(mnemonicLen)
		require.NoError(t, err)
		require.Equal(t, len(entropy), int(mnemonicLen)*32/24)
	}
	fn_test(12)
	fn_test(24)
}
...
func TestMnemonicToEntropy(t *testing.T) {
	fn_test := func(mnemonic string, entropyExpect []byte) {
		entropy, err := MnemonicToEntropy(mnemonic)
		require.NoError(t, err)
		require.Equal(t, entropy, entropyExpect)
	}
	// len 12
	fn_test("bitter gadget energy suit dice toward shuffle kite educate fish run repeat",
		[]byte{22, 203, 213, 40, 108, 131, 215, 204, 49, 219, 217, 70, 138, 246, 245, 91})
	fn_test("shoot honey melt cruel model act program harsh giggle party frozen stumble",
		[]byte{198, 141, 166, 42, 154, 72, 232, 4, 234, 251, 74, 97, 244, 17, 118, 235})
	fn_test("youth year shuffle miracle hello brave mobile lizard legend focus until utility",
		[]byte{255, 127, 223, 29, 198, 182, 172, 54, 163, 156, 22, 127, 139, 71, 184, 248})
	fn_test("grant fade apology genius coach merry very entry sure husband prison cash",
		[]byte{101, 170, 56, 41, 176, 114, 203, 23, 124, 186, 93, 218, 13, 254, 172, 17})
	fn_test("sponsor pilot grain lift best ticket blame coconut strike flag mirror thrive",
		[]byte{210, 116, 157, 150, 64, 177, 85, 195, 69, 201, 103, 215, 11, 6, 54, 112})
	fn_test("head obvious slab arrange dentist media gentle cousin yellow giraffe case shrimp",
		[]byte{106, 19, 23, 43, 134, 35, 169, 20, 216, 73, 139, 255, 12, 68, 140, 227})

	// len 24
	fn_test("jungle wrist shy lottery grid horse reveal video close disorder faint mouse case wool bird people solve feed tennis force hurdle view reunion fit",
		[]byte{121, 31, 207, 30, 66, 6, 102, 219, 174, 31, 158, 43, 167, 237, 71, 200, 82, 51, 250, 133, 165, 22, 206, 234, 151, 124, 173, 134, 249, 231, 238, 18})
	fn_test("erosion bundle pupil dice ginger witness host grab long pumpkin layer time delay true vague consider draft album confirm trash tired object just raven",
	...
	fn_test("muscle thing flower van fatigue office thumb enlist hammer rely bronze rotate wise jelly business army disease town either real agree alert honey soft",
		[]byte{145, 156, 21, 102, 120, 149, 61, 51, 56, 90, 86, 104, 150, 172, 114, 222, 31, 198, 239, 135, 192, 96, 63, 28, 201, 28, 89, 112, 80, 12, 91, 78})
	fn_test("report ride fall female lawsuit galaxy floor squirrel put find ancient dream poverty young bronze poem paper place chair cheese grace visual cry menu",
	...
	fn_test("warfare maple loyal marine tennis vacuum nephew answer online country bid pilot mask ostrich jealous jelly theme deliver leader west book sea fuel hero",
		[]byte{247, 16, 242, 18, 68, 13, 243, 225, 37, 16, 77, 154, 230, 32, 88, 82, 120, 135, 57, 221, 227, 190, 224, 7, 65, 250, 124, 177, 153, 131, 215, 123})
	fn_test("body smooth hero lyrics plastic glare behave invest ritual lounge hedgehog used traffic gauge juice sign artist merge annual diamond code drastic meat hip",
...
func TestEntropyToMnemonic(t *testing.T) {
	fn_test := func(entropy []byte, mnemonicExpect string) {
		mnemonic, err := EntropyToMnemonic(entropy)
		require.NoError(t, err)
		require.Equal(t, mnemonic, mnemonicExpect)
	}
	// len 12
	fn_test([]byte{22, 203, 213, 40, 108, 131, 215, 204, 49, 219, 217, 70, 138, 246, 245, 91},
		"bitter gadget energy suit dice toward shuffle kite educate fish run repeat")
	fn_test([]byte{198, 141, 166, 42, 154, 72, 232, 4, 234, 251, 74, 97, 244, 17, 118, 235},
		"shoot honey melt cruel model act program harsh giggle party frozen stumble")
	fn_test([]byte{255, 127, 223, 29, 198, 182, 172, 54, 163, 156, 22, 127, 139, 71, 184, 248},
		"youth year shuffle miracle hello brave mobile lizard legend focus until utility")
	fn_test([]byte{101, 170, 56, 41, 176, 114, 203, 23, 124, 186, 93, 218, 13, 254, 172, 17},
		"grant fade apology genius coach merry very entry sure husband prison cash")
	fn_test([]byte{210, 116, 157, 150, 64, 177, 85, 195, 69, 201, 103, 215, 11, 6, 54, 112},
		"sponsor pilot grain lift best ticket blame coconut strike flag mirror thrive")
	fn_test([]byte{106, 19, 23, 43, 134, 35, 169, 20, 216, 73, 139, 255, 12, 68, 140, 227},
...
func TestMnemonicToSeed(t *testing.T) {
	fn_test := func(mnemonic string, pw string, seedExpect []byte) {
		seed, err := MnemonicToSeedWithPw(mnemonic, pw)
		require.NoError(t, err)
		require.Equal(t, seed, seedExpect)
	}
...
func TestMasterGet(t *testing.T) {
	fn_test := func(seed []byte, pw []byte, keyExpect []byte, chaincodeExpect []byte) {
		privateKey, chaincode, err := SeedToMasterKeyWithPw(seed, pw)
		require.NoError(t, err)
		require.Equal(t, privateKey, keyExpect)
		require.Equal(t, chaincode, chaincodeExpect)
	}
...
func TestChildGet(t *testing.T) {
	fn_test := func(key, chaincode []byte, deriveType DeriveType, idx uint32, keyExpect []byte, chaincodeExpect []byte) {
		childKey, childChaincode, err := Derive(deriveType, key, chaincode, idx)
		require.NoError(t, err)
		require.Equal(t, childKey, keyExpect)
		require.Equal(t, chaincode, chaincodeExpect)
		// 두 값 비교
		if bytes.Equal(childKey, keyExpect) != true {
			t.Errorf("invalid child key | result : %v | expect : %v", childKey, keyExpect)
		}
		if bytes.Equal(childChaincode, chaincodeExpect) != true {
			t.Errorf("invalid child chaincode | result : %v | expect : %v", childChaincode, chaincodeExpect)
		}
	}
...
func TestMnemonicEntropy(t *testing.T) {
	fn_test := func(mnemonicLen int) {
		...
		// string -> bt_idx
		entropyNew, err := MnemonicToEntropy(mnemonic)
		require.NoError(t, err)

		// bt_idx -> string
		mnemonicNew, err := EntropyToMnemonic(entropyNew)
		require.NoError(t, err)

		require.Equal(t, entropy, entropyNew)
		require.Equal(t, mnemonic, mnemonicNew)
	}
...
func TestSeedMasterKey(t *testing.T) {
	// btcd 라이브러리 extendedkey 와 같은 지 비교
	fn_test := func(mnemonic, pwSeed, pwMasterKey string) {
		seed, err := MnemonicToSeedWithPw(mnemonic, pwSeed)
		require.NoError(t, err)

		childKey, childChaincode, err := SeedToMasterKeyWithPw(seed, []byte(pwMasterKey))
		require.NoError(t, err)

		// btcd 라이브러리
		seedFromLib, err := bip39.NewSeedWithErrorChecking(mnemonic, pwSeed)
		require.NoError(t, err)

		extendedKey, err := hdkeychain.NewMaster(seedFromLib, &chaincfg.MainNetParams)
		require.NoError(t, err)

		privKey, err := extendedKey.ECPrivKey()
		require.NoError(t, err)

		childKeyFromLib := privKey.Serialize()
		childChaincodeFromLib := extendedKey.ChainCode()

		require.Equal(t, seed, seedFromLib)
		require.Equal(t, childKey, childKeyFromLib)
		require.Equal(t, childChaincode, childChaincodeFromLib)
	}

	// 고정 니모닉 사용
	{
		fn_test(mnemonic12, pwSeed, pwBitcoinMasterKey)
		fn_test(mnemonic24, pwSeed, pwBitcoinMasterKey)
	}

	// 임의로 생성된 니모닉 사용
	{
		bt_mnemonic__rand, _ := EntropyNew(12)
		s_mnemonic__rand, _ := EntropyToMnemonic(bt_mnemonic__rand)
		fn_test(s_mnemonic__rand, pwSeed, pwBitcoinMasterKey)

		bt_mnemonic__rand, _ = EntropyNew(24)
		s_mnemonic__rand, _ = EntropyToMnemonic(bt_mnemonic__rand)
		fn_test(s_mnemonic__rand, pwSeed, pwBitcoinMasterKey)
	}
}
